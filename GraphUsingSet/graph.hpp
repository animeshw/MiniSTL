...
namespace ani{
    ...
    class vertex{
        ...
        public:
            ...
            const set<neighbor>& get_neighbors() const{ return neighbors; }
    ...
    class graph{
        ...
        public:
            ...
            void add_edge(int start_vertex, int end_vertex, double weight = INFINITY){
                ani::set<vertex>::iterator start_iter;
                ani::set<vertex>::iterator end_iter;
            ...
            void remove_vertex(int del_vertex){
                ani::set<vertex>::iterator del_iter;
                ani::set<vertex>::iterator complimentry_vert_iter;
                ani::set<neighbor>::const_iterator nbr_iter;

                del_iter = vertices.find(del_vertex);
                if(del_iter == vertices.end())
                    throw graph_vertex_invalid();
            ...
            void print(const char* msg) const{
                ...
                for(set<vertex>::const_iterator vert_iter = vertices.begin(); vert_iter != vertices.end(); ++vert_iter){
                    std::cout << "[" << vert_iter->get_id() << "]\t->\t";
                    for(set<neighbor>::const_iterator nbr_iter = vert_iter->get_neighbors().begin(); nbr_iter != vert_iter->get_neighbors().end(); ++nbr_iter)
            ...
            void dfs(int start_vertex) const{
                ...
                std::cout << "[START]<->";
                set<vertex>::const_iterator citer = vertices.find(start_vertex);
            ...
            void dijkstra(int start_vertex) const {
                set<vertex>::const_iterator start_citer;
                priority_queue<vertex, ani::vector<vertex>, vertex_comparator> pq_vertices;
                double weight;

                for(set<vertex>::const_iterator vert_citer = vertices.begin(); vert_citer != vertices.end(); ++vert_citer){
                ...
                while(!pq_vertices.empty()){
                    ...
                    pq_vertices.pop();
                    for(set<neighbor>::const_iterator nbr_citer = min_vert.get_neighbors().begin(); nbr_citer != min_vert.get_neighbors().end(); ++nbr_citer){
                        set<vertex>::const_iterator vert_citer = vertices.find(nbr_citer->get_id());
            ...
            void bellman_ford(int start_vertex) const{
                ...
                for(size_t i = 1; i < nr_vertices; ++i){
                    for(set<vertex>::const_iterator vert_citer = vertices.begin(); vert_citer != vertices.end(); ++vert_citer){
                        for(set<neighbor>::const_iterator nbr_citer = vert_citer->get_neighbors().begin(); nbr_citer != vert_citer->get_neighbors().end(); ++nbr_citer){
                            set<vertex>::const_iterator complimentry_vert_iter = vertices.find(nbr_citer->get_id());
                            if(complimentry_vert_iter != vertices.end()){
            ...
            void prim(int start_vertex) const{
                set<vertex>::const_iterator start_citer;
                set<vertex>::const_iterator min_vert_citer;
                priority_queue<vertex, ani::vector<vertex>, vertex_comparator> pq_vertices;
                double weight;
                ...
                while(!pq_vertices.empty()){
                    set<vertex>::const_iterator vert_citer = vertices.find(pq_vertices.top().get_id());
                    ...
                    for(set<neighbor>::const_iterator nbr_citer = min_vert_citer->get_neighbors().begin(); nbr_citer != min_vert_citer->get_neighbors().end(); ++nbr_citer){
                ...
                std::cout << "[START]<->";
                for(set<vertex>::const_iterator citer = vertices.begin(); citer != vertices.end(); ++citer) {
                    if(citer->pred_id != -1){
                ...
                for(set<vertex>::const_iterator vert_citer = vertices.begin(); vert_citer != vertices.end(); ++vert_citer){
                    for(set<neighbor>::const_iterator nbr_citer = vert_citer->get_neighbors().begin(); nbr_citer != vert_citer->get_neighbors().end(); ++nbr_citer){
                        if(nbr_citer->get_color() != GRAY){
                            ...
                            set<vertex>::const_iterator complimentry_vert_iter = vertices.find(nbr_citer->get_id());
                            if(complimentry_vert_iter != vertices.end()){
                                set<neighbor>::const_iterator complimentry_nbr_iter = complimentry_vert_iter->get_neighbors().find(vert_citer->get_id());
        ...
        private:
            void print_shortest_path(const vertex& vert) const{
                set<vertex>::const_iterator citer;
            ...
            void _dfs(const vertex& vert) const{
                ...
                for(set<neighbor>::const_iterator nbr_citer = vert.get_neighbors().begin(); nbr_citer != vert.get_neighbors().end(); ++nbr_citer){
                    set<vertex>::const_iterator vert_citer = vertices.find(nbr_citer->get_id());
            ...
            void _bfs(const vertex& vert)const{
                ...
                while(!queue.empty()){
                    ...
                    for(set<neighbor>::const_iterator nbr_citer = queue.front().get_neighbors().begin(); nbr_citer != queue.front().get_neighbors().end(); ++nbr_citer){
                        set<vertex>::const_iterator vert_citer = vertices.find(nbr_citer->get_id());
                        if(vert_citer != vertices.end() && vert_citer->get_color() == WHITE){
                            vert_citer->col = GRAY;
